        ...
        @if (p.items.length === 0) {
          ...
          @if (totalPages() > 1) {
            <nav class="pager" aria-label="Sivutus">
              <button type="button" [disabled]="page().page <= 1" (click)="goPage(page().page - 1)">‹ Edellinen</button>
              <span>Sivu {{ page().page }} / {{ totalPages() }}</span>
              <button type="button" [disabled]="page().page >= totalPages()" (click)="goPage(page().page + 1)">Seuraava ›</button>
            </nav>
          }
        }
      }
