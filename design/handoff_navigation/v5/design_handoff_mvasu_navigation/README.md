...
```
claude --continue
> Lue design_handoff_mvasu_navigation/README.md sekä NAVIGATION.md,
> DEVEXTREME.md, EXISTING_ENTITIES.md, SCREENS.md, BEHAVIOR.md ja
> BACKEND.md. NAVIGATION.md sisältää virallisen XAF-rakenteen
> (Model.xafml) — käytä siellä olevia ViewId- ja kenttänimiä
> ehdottomana totuutena. Toteuta
...
├── NAVIGATION.md                   ← ⭐⭐ TÄRKEIN — virallinen XAF-navigaatio + Tehtava-kentät Model.xafml-tiedostosta
├── Model.xafml                     ← XAF Application Model (raakatiedosto, viittauksia varten)
...
> **Lukemisjärjestys**: aloita README → **NAVIGATION** (XAF-totuus) → DEVEXTREME (kova rajoite) → EXISTING_ENTITIES (mitä on jo olemassa) → SCREENS (mitä rakennetaan) → BEHAVIOR → BACKEND. `NAVIGATION.md` ja `EXISTING_ENTITIES.md` ohittaa `BACKEND.md`:n SQL-luonnokset jos ristiriita; `DEVEXTREME.md` ohittaa `SCREENS.md`:n HTML-luonnokset komponenttivalinnoissa.
